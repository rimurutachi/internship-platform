...
interface UndoRedoState {
  documentId: string;
  userId: string;
  undoStack: DocumentChange[];
  redoStack: DocumentChange[];
  currentIndex: number;
}
...
export const collaborationService = {
   ...
   */
  async endSession(documentId: string, userId: string, sessionId: string): Promise<boolean> {
   ...
   */
  async recordChange(change: DocumentChange): Promise<boolean> {
   ...
   */
  async getChangeHistory(documentId: string, limit: number = 100): Promise<DocumentChange[]> {
   ...
   */
  async undo(documentId: string, userId: string): Promise<DocumentChange | null> {
    ...
    try {
      ...
      const inverseChange: DocumentChange = {
        documentId,
        userId,
   ...
   */
  async redo(documentId: string, userId: string): Promise<DocumentChange | null> {
    ...
    try {
      ...
      const redoChange: DocumentChange = {
        documentId,
        userId,
        operation: change.operation,
        index: change.index,
  ...
  async getActivityStats(
    ...
    totalChanges: number;
    changesByUser: { userId: string; userName: string; changeCount: number }[];
    operationTypes: { operation: string; count: number }[];
    ...
    try {
      ...
      // Group by user
      const changesByUserMap = new Map<string, { userId: string; userName: string; count: number }>();
