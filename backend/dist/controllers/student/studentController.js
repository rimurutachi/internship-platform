...
class StudentController {
     ...
     */
    async getCurrentInternship(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ success: false, error: 'Unauthorized' });
            }
            const internship = await studentService_1.default.getCurrentInternship(userId);
            if (!internship) {
     ...
     */
    async getInternshipTimeline(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ success: false, error: 'Unauthorized' });
    ...
    async getProgress(req, res, next) {
        try {
            ...
            const progress = studentService_1.default.calculateProgressMetrics(internship.start_date, internship.end_date);
    ...
    async getEvaluations(req, res, next) {
        try {
            ...
            const result = await studentService_1.default.getEvaluations(internship.id, limit, offset);
     ...
     */
    async getSkillsAssessment(req, res, next) {
        try {
            const userId = req.user?.id;
            if (!userId) {
                return res.status(401).json({ success: false, error: 'Unauthorized' });
            ...
            const assessment = await studentService_1.default.getSkillsAssessment(internship.id);
    ...
    async getRequiredDocuments(req, res, next) {
        try {
            ...
            const status = await studentService_1.default.getRequiredDocumentsStatus(internship.id);
     ...
     */
    async messageMentor(req, res, next) {
        try {
            const userId = req.user?.id;
    ...
    async getDashboardOverview(req, res, next) {
        try {
            ...
            const dashboardData = await studentService_1.default.getDashboardData(userId);
            if (!dashboardData) {
