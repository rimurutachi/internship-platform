...
class SettingsController {
    async getProfile(req, res) {
        console.log('[admin/settings] GET /profile', { userId: req.user?.id });
        try {
    ...
    async updateProfile(req, res) {
        console.log('[admin/settings] PATCH /profile', { userId: req.user?.id, body: req.body });
        try {
            const userId = req.user?.id;
            if (!userId)
                return res.status(401).json({ error: 'Unauthorized' });
