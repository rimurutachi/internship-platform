...
export const setupMessageHandlers = (io: Server, socket: AuthenticatedSocket) => {
  ...
  socket.on("send_message", async (data: { receiverId: string, content: string }, callback) => {
    try {
      ...
      const validContacts = await messageService.getContacts(userId, userRole);
      const receiverRole = validContacts.find(c => c.id === data.receiverId)?.role;
      const isAllowed = !!receiverRole;
      
      if (!isAllowed) {
        if (callback) callback({ error: "You are not allowed to message this user." });
        return;
      }

      const savedMessage = await messageService.sendMessage(userId, data.receiverId, data.content);
  ...
  socket.on("mark_messages_read", async (data: { messageIds: string[] }, callback) => {
    try {
      ...
      await messageService.markAsRead(data.messageIds);
