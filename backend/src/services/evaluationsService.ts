...
export class EvaluationsService {
  ...
  async getMetricsBySupervisor() {
    try {
      ...
      const supervisorMetrics = Array.from(supervisorMap.values()).map(
        (supervisor) => {
          const evals = supervisor.evaluations;
          const processedEvals = evals.filter(
            (e: any) => e.status === 'processed' || e.status === 'approved'
          );

          const avgRatings =
            evals.length > 0
              ? evals.map((e: any) => this.calculateAverageRating(e))
              : [0];
          const avgRating =
            avgRatings.reduce((a: number, b: number) => a + b, 0) /
            avgRatings.length;
   ...
   */
  async generateQualityReport(startDate: Date, endDate: Date) {
    try {
      const { data: evaluations } = await supabase
        .from('evaluations')
        .select('*')
