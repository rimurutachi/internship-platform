...
class SecurityController {
  ...
  // Get login attempts
  static async getLoginAttempts(req: Request, res: Response) {
    try {
    ...
    try {
      ...
      if (format === 'csv') {
        const csv = [
          ...
          ...(data || []).map((log: any) =>
            `${log.id},${log.method},${log.path},${log.status_code},${log.response_time_ms},${log.user_id || ''},${log.ip_address || ''},${log.created_at}`
          )
        ].join('\n');
