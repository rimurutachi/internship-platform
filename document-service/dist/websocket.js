 ...
 */
async function checkDocumentAccess(documentId, userId, userRole) {
    try {
        ...
        if (!dac)
            return false;
        if (dac.expires_at && new Date(dac.expires_at) < new Date())
            return false;
        return true;
    }
    catch (error) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("document:join", async ({ documentId, userId, userName, userEmail, userColor }) => {
            try {
                ...
                currentSession = await collaborationService_1.default.initializeSession(documentId, socket.user.id, userName || socket.user.email?.split('@')[0] || "Unknown", userEmail || socket.user.email || "unknown@example.com");
                // Get or create Y.Doc
