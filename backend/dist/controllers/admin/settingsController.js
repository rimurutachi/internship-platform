...
class SettingsController {
    ...
    async updateProfile(req, res) {
        try {
            ...
            if (phone && phone.trim() !== '') {
                try {
                    settingsService_1.default.validatePhone(phone);
                }
                catch (error) {
            ...
            // Log activity
            await settingsService_1.default.logActivity(userId, 'profile_updated', { first_name, last_name, phone });
            res.status(200).json({
                id: updatedUser.id,
    ...
    async uploadAvatar(req, res) {
        upload(req, res, async (err) => {
            try {
                ...
                const avatarUrl = await settingsService_1.default.uploadToStorage(req.file.buffer, 'avatars', filePath, req.file.mimetype);
                ...
                await supabase
                    ...
                    .eq('id', userId);
                // Log activity
                await settingsService_1.default.logActivity(userId, 'avatar_uploaded', { avatar_url: avatarUrl });
    ...
    async updatePlatformSettings(req, res) {
        try {
            const userId = req.user?.id;
            const { platform_name, platform_url, support_email, max_file_upload_mb, session_timeout_minutes, platform_timezone, platform_announcement } = req.body;
            // Validation
            ...
            try {
                settingsService_1.default.validateUrl(platform_url);
            }
            catch (error) {
                res.status(400).json({ error: error.message });
                return;
            ...
            try {
                settingsService_1.default.validateEmail(support_email);
            }
            catch (error) {
                res.status(400).json({ error: error.message });
                return;
            ...
            try {
                settingsService_1.default.validateTimezone(platform_timezone);
            }
            catch (error) {
                res.status(400).json({ error: error.message });
                return;
            ...
            for (const setting of settingsToUpdate) {
                await settingsService_1.default.updateSetting(setting.key, setting.value, setting.type, setting.description);
            }
            // Log activity
            await settingsService_1.default.logActivity(userId, 'platform_settings_updated', req.body);
            res.status(200).json({
                platform_name,
                platform_url,
                support_email,
    ...
    async updateNotificationSettings(req, res) {
        try {
            const userId = req.user?.id;
            const { notify_system_alerts, notify_user_management, notify_security_events, notify_platform_performance } = req.body;
            // Validation
            ...
            // Log activity
            await settingsService_1.default.logActivity(userId, 'notification_settings_updated', req.body);
            res.status(200).json({
                notify_system_alerts,
                notify_user_management,
                notify_security_events,
    ...
    async updateAdvancedSettings(req, res) {
        try {
            ...
            // Update settings
            await settingsService_1.default.updateSetting('maintenance_mode_enabled', maintenance_mode_enabled, 'system', 'Maintenance mode enabled flag');
            await settingsService_1.default.updateSetting('api_rate_limit_per_minute', api_rate_limit_per_minute, 'system', 'API rate limit per minute');
            // Log activity
            await settingsService_1.default.logActivity(userId, 'advanced_settings_updated', req.body);
            res.status(200).json({
                maintenance_mode_enabled,
                api_rate_limit_per_minute
            });
    ...
    async triggerBackup(req, res) {
        try {
            const userId = req.user?.id;
            const backupResult = await settingsService_1.default.triggerBackup();
            // Log activity
            await settingsService_1.default.logActivity(userId, 'backup_triggered', backupResult);
    ...
    async clearCache(req, res) {
        try {
            const userId = req.user?.id;
            const result = await settingsService_1.default.clearSystemCache();
            // Log activity
            await settingsService_1.default.logActivity(userId, 'cache_cleared', result);
    ...
    async getSystemHealth(req, res) {
        try {
            const health = await settingsService_1.default.getSystemHealth();
    ...
    async getTimezones(req, res) {
        try {
            const timezones = settingsService_1.default.getTimezones();
