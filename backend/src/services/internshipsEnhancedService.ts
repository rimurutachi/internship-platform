...
export class InternshipsEnhancedService {
  /**
   * Auto-decrement/increment company current_students
   */
  static async updateCompanyStudentCount(company_id: string, delta: number): Promise<void> {
   ...
   */
  static async generateAutoReminders(internship_id: string): Promise<InternshipReminder[]> {
    ...
    const reminders: Omit<InternshipReminder, 'id'>[] = [];
    ...
    if (reminders.length > 0) {
      ...
      return data as InternshipReminder[];
    }
   ...
   */
  private static async sendReminderNotification(reminder: any): Promise<void> {
   ...
   */
  static async exportInternships(internship_ids: string[], format: 'csv' | 'json' | 'excel'): Promise<any> {
   ...
   */
  private static convertToCSV(internships: any[]): string {
    const fields = [
      'id',
      'student.name',
      'student.email',
   ...
   */
  private static convertToExcel(internships: any[]): string {
   ...
   */
  static async getDocumentCompletionRate(internship_ids: string[]): Promise<Record<string, number>> {
    ...
    const completionByInternship: Record<string, number> = {};
    ...
    internship_ids.forEach(id => {
      ...
      const completedRequired = docs.filter(
        d => requiredTypes.includes(d.type) && d.status !== 'draft'
      ).length;
   ...
   */
  static async getCompanyCapacityAnalytics(): Promise<any> {
   ...
   */
  static async validateCompanyCapacity(company_id: string): Promise<{ canAccept: boolean; message?: string }> {
