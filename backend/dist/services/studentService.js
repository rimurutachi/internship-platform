...
class StudentService {
     ...
     */
    async getAIInsights(internshipId) {
        ...
        const avgRating = evaluations.reduce((sum, e) => sum + (e.rating_overall || 0), 0) / evaluations.length;
     ...
     */
    async getRequiredDocumentsStatus(internshipId) {
     ...
     */
    async getCurrentInternship(studentId) {
    ...
    async getEvaluations(internshipId, limit = 10, offset = 0) {
      ...
      `, { count: 'exact' })
        ...
        const avgRating = evaluations && evaluations.length > 0
            ? (evaluations.reduce((sum, e) => sum + (e.rating_overall || 0), 0) / evaluations.length).toFixed(1)
            : '0';
     ...
     */
    async getSkillsAssessment(internshipId) {
        const { data: evaluations, error } = await supabase
            .from('evaluations')
        ...
        const avgTechnical = evaluations.reduce((sum, e) => sum + (e.rating_technical || 0), 0) / evaluations.length;
        const avgCommunication = evaluations.reduce((sum, e) => sum + (e.rating_communication || 0), 0) / evaluations.length;
        const avgWorkEthic = evaluations.reduce((sum, e) => sum + (e.rating_work_ethic || 0), 0) / evaluations.length;
        ...
        const avgOverall = evaluations.reduce((sum, e) => sum + (e.rating_overall || 0), 0) / evaluations.length;
        return {
            skills,
