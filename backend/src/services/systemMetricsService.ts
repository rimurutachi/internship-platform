...
export interface SystemEvent {
  id: string;
  type: string;
  service?: string; // Nullable in your schema
  message: string;
  error_code?: string; // Added from your schema
...
class SystemMetricsService {
  ...
  async getServiceStatus(): Promise<ServiceStatus[]> {
    ...
    services.push({
      ...
      cpuUsage: Number((os.loadavg()[0] * 10).toFixed(1)), // Approximate CPU usage
      lastHealthCheck: now
    });
    ...
    try {
      ...
      const aiResponseTime = performance.now() - aiStart;
    ...
    services.push({
      ...
      cpuUsage: Number((os.loadavg()[0] * 5).toFixed(1)),
      lastHealthCheck: now
    });
   ...
   */
  async getRecentEvents(limit: number = 20, severity?: string, type?: string): Promise<SystemEvent[]> {
    try {
  ...
  async logEvent(event: {
    type: string;
    service?: string; // Optional since it's nullable in schema
    message: string;
    error_code?: string; // Added to match schema
    severity: 'info' | 'warning' | 'error' | 'critical';
    count?: number;
    metadata?: any;
  }) {
   ...
   * Get metrics trend
   */
  async getMetricsTrend(metric: string, hours: number = 24) {
    try {
   ...
   */
  private generateMockTrendValue(metric: string, hoursAgo: number): number {
