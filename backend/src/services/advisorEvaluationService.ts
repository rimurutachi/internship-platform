 ...
 */
async function triggerAIAnalytics(evaluationId: string, universityId: string) {
  try {
    ...
    const evaluationsForAI = approvedEvaluations.map((evaluation: any) => ({
      evaluation_id: evaluation.id,
      text: `${evaluation.supervisor_comments || ''} ${evaluation.advisor_comments || ''}`.trim(),
      ratings: evaluation.criterion_scores?.reduce((acc: any, cs: any) => {
    ...
    const analyticsResult = await response.json() as {
      status: string;
