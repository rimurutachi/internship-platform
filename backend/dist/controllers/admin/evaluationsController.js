...
class EvaluationsController {
    constructor() {
         ...
         */
        this.getAIResults = async (req, res) => {
            try {
                const { id } = req.params;
                const { data: evaluation, error } = await supabase
                    .from('evaluations')
         ...
         */
        this.validateSentiment = async (req, res) => {
            try {
                const { id } = req.params;
         ...
         */
        this.validateFeatures = async (req, res) => {
            try {
                const { id } = req.params;
         ...
         */
        this.validateBias = async (req, res) => {
            try {
                const { id } = req.params;
         ...
         */
        this.overrideGrade = async (req, res) => {
            try {
                const { id } = req.params;
                ...
                const originalGrade = evaluation.final_grade || evaluation.recommended_grade;
         ...
         */
        this.rejectEvaluation = async (req, res) => {
            try {
                const { id } = req.params;
         ...
         */
        this.requestReprocess = async (req, res) => {
            try {
                const { id } = req.params;
        ...
        this.bulkApprove = async (req, res) => {
            try {
                ...
                if (!Array.isArray(evaluation_ids) ||
                ...
                for (const id of evaluation_ids) {
                    try {
                        ...
                        const gradeToSet = evaluation.final_grade || evaluation.recommended_grade;
                        if (!gradeToSet) {
                            failedCount++;
        ...
        this.bulkExport = async (req, res) => {
            try {
                ...
                if (format === 'csv') {
                    ...
                    res.setHeader('Content-Disposition', `attachment; filename=evaluations-export-${Date.now()}.json`);
                }
                res.send(exportData);
            }
            catch (error) {
...
//# sourceMappingURL=evaluationsController.js.map