...
export default function CurrentInternship() {
  ...
  const [hoursSummary, setHoursSummary] =
    useState<InternshipHoursSummary | null>(null);
  ...
  const fetchHoursData = async (internshipId: string) => {
    try {
      ...
      const [summaryResult, breakdownResult, reportsResult, dtrResultQuery] = await Promise.all(
        [
      ...
      if (reportsResult.data) {
        console.log(
          "✅ [Daily Reports] Setting daily reports:",
          reportsResult.data,
        );
  ...
  const loadInternshipData = async () => {
    try {
      ...
      console.log(
        "🔵 [Current Internship] API response status:",
        response.status,
      );
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.warn(
            "⚠️ [Current Internship] No active internship found for user:",
            user.id,
          );
      ...
      const formattedInternship: InternshipData = {
        ...
        supervisor: internshipData.supervisor
          ? {
              name:
                internshipData.supervisor.name ||
                `${internshipData.supervisor.first_name} ${internshipData.supervisor.last_name}`,
        ...
        advisor: internshipData.advisor
          ? {
              name:
                internshipData.advisor.name ||
                `${internshipData.advisor.first_name} ${internshipData.advisor.last_name}`,
      ...
      <div className="space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2 text-lg">
            Track your internship progress and milestones
          </p>
        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-lg text-muted-foreground">
                    {hoursSummary.total_hours_worked} /{" "}
                    {hoursSummary.required_hours} hours completed
                  </span>
              ...
              {hoursSummary.is_completed && (
                <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/30 text-green-700 dark:text-green-400">
                  <p className="font-medium text-lg">
                    ✅ Congratulations! You've completed your required
                    internship hours!
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="font-semibold text-foreground text-lg">
                    {internship.supervisor?.name || "Not assigned"}
                  </div>
                  <div className="text-base text-muted-foreground">
                    {internship.supervisor?.email || "No email"}
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="bg-card border border-border">
            ...
            <CardContent>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="font-semibold text-foreground text-lg">
                    {internship.advisor?.name || "Not assigned"}
                  </div>
                  <div className="text-base text-muted-foreground">
                    {internship.advisor?.email || "No email"}
                  </div>
                </div>
              </div>
            </CardContent>
